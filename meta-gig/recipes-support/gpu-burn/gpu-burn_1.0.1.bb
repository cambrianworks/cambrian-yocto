...
SRCREV = "3ead140434da9473582b68452f7115967a7a0581"
...
SRC_URI += " \
    file://compare-orin.fatbin \
"

# Default location for the compare.ptx file
GPU_BURN_BINDIR = "${bindir}"
...
do_compile() {
    ...
    oe_runmake \
        ...
        CC="${CC}" \
        CUDAPATH=${CUDA_PATH} \
        CFLAGS="${CFLAGS} -I${STAGING_DIR_TARGET}${CUDA_INSTALL_PATH}/include -DCOMPARE_PTX_PATH='\"${GPU_BURN_BINDIR}/compare.ptx\"'" \
...
do_install() {
    ...
    install -d ${D}${GPU_BURN_BINDIR}
    install -m 0644 ${WORKDIR}/compare-orin.fatbin ${D}${GPU_BURN_BINDIR}/compare.ptx
}

FILES:${PN} += "${bindir}/gpu-burn"
FILES:${PN} += "${GPU_BURN_BINDIR}/compare.ptx"
